# CLAUDE_CUSTOM_APP.md

**One-shot guide for creating Verily Workbench custom apps.**
...
`src/clinical-abstraction-demo/` (branch: yp_ac_clin)

### Complex multi-service app with Caddy
`src/playground/` (public repo: verily-src/workbench-app-devcontainers)

### Pre-built image examples (no Dockerfile needed)
- `src/vscode/` - VS Code Server on port 8443
- `src/r-analysis/` - RStudio on port 8787
